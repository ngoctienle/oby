              ...
              <p
                className={`font-medium @520:fs-12 fs-10 ${
                  router.pathname === hrefPath.home
                    ? `${customClass.COMMON_GRADIENT} inline-block text-transparent bg-clip-text`
                    : 'text-oby-676869'
                }`}
              >
                Trang chủ
          ...
          <OBYButton variant={'ghost'} size={'ghost'} asChild className='flex flex-col items-center'>
            <OBYLink href={hrefPath.cartPage}>
              <div className='relative w-6 h-6'>
                ...
                />
                {cartData && (
                  <p className='absolute flex items-center justify-center -top-0.5 -right-1 w-4.5 h-4.5 fs-10 ${customClass.COMMON_GRADIENT} text-white rounded-full'>
                    {cartData.items_qty}
                  </p>
                )}
              </div>
