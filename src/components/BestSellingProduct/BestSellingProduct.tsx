...
export const BestSellingProduct = () => {
  ...
  const renderSection = () => {
    if (isLoading) {
      return (
        <div className='container pt-8 pb-4 flex flex-col'>
    ...
    if (!isLoading && bestSellingProduct) {
      if (bestSellingProduct?.data.items.length === 0) return <div className='container pt-8 pb-4 flex flex-col'></div>
      return (
        <div className='container pt-8 pb-4 flex flex-col'>
          <h2 className='text-oby-primary fs-14 font-normal'>SẢN PHẨM</h2>
          <p className='text-[#222324] fs-26 font-bold mt-2'>TOP SẢN PHẨM BÁN CHẠY</p>
          ...
          <div className='w-[194px] self-center'>
            <GradientButtonLink url={`${hrefPath.catePage}/top-san-pham-ban-chay-45`} btnText='XEM TẤT CẢ' />
          </div>
        </div>
      )
    }
